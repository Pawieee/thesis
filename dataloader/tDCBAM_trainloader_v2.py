...
def preprocess_image(img, img_size=(224, 224)):
    ...
    inversion, tight cropping, aspect-aware resizing, and padding.
    Returns an ImageNet-normalized PyTorch tensor (C, H, W).
    """
    if img is None:
        return None

    # 1. Convert PIL to Numpy array if necessary
    ...
    target_size = img_size[0] # Assuming square 224x224
    h_c, w_c = img_crop.shape
    scale = target_size / max(h_c, w_c)
    nw, nh = int(w_c * scale), int(h_c * scale)
    
    # Fallback for extremely distorted edge cases
    if nw == 0 or nh == 0:
    ...
    # 7. RGB Conversion & Normalization
    img_rgb = cv2.cvtColor(canvas, cv2.COLOR_GRAY2RGB)
    img_float = img_rgb.astype("float32") / 255.0
    
    # Convert to Tensor (C, H, W) and apply ImageNet standardization
