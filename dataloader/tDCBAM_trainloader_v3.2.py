...
def preprocess_image(img, img_size=(224, 224), augment=False): # <--- ADDED FLAG
    ...
    # --- ONLY APPLY EROSION/DILATION IF AUGMENT IS TRUE ---
...
def get_pretraining_transforms(input_shape=(224, 224), preprocess=False, augment=True):
    ...
    if augment:
        transform_list.extend([
            ...
            # If preprocessing converts to white-on-black:
            transforms.RandomRotation(degrees=20, fill=0), # 0 matches the inverted black background
            transforms.RandomAffine(degrees=0, translate=(0.2, 0.2), fill=0),
        ])

    if preprocess:
        # This function handles cropping, morphology, padding, AND tensor conversion
        transform_list.append(transforms.Lambda(lambda img: preprocess_image(img, input_shape, augment)))
    else:
        # Standard path for non-preprocessed images
        transform_list.extend([
...
def get_baseline_transforms(input_shape=(224, 224), preprocess=False, augment=True):
    ...
    if preprocess:
        # Lambda handles everything; do NOT add ToTensor/Normalize after this
        ...
        # Standard path needs the explicit conversions
        train_list.extend([
            transforms.Resize(input_shape),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
