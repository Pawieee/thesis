...
class SignatureEpisodeDataset(Dataset):
    ...
    def __init__(self, split_file, root_dir=None, mode='train', k_shot=1, 
        ...
        if len(self.users) == 0:
            raise ValueError(f"Dataset is empty! Check the JSON file {split_file} and mode {mode}")
            
        first_val = self.data[self.users[0]]
        if not isinstance(first_val, dict):
            raise ValueError(f"Invalid JSON structure. User {self.users[0]} does not contain image dictionary.")
