...
# FOR TESTING THE BASELINE PAIRWISE 

    def extract_features(self, x):
            """
            Bypasses the final classification layer to return the raw 1024D embedding 
            from the DenseNet backbone. Used exclusively for pairwise evaluation 
            to expose the true embedding space of the baseline model.
            """
            if self.baseline:
                x = self.backbone(x)
                x = self.post_norm_relu(x)
                x = self.avgpool(x)
                x = torch.flatten(x, 1)
                # Returns raw 1024D features (Normalization happens in the eval loop)
                return x
            else:
                # Stem
                x = self.initial_layers(x)
